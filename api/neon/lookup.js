  ...
  if (tParam === 'jira-projects') {
    ...
    if (req.method === 'GET') {
      try {
        ...
        const rows = await sql`SELECT id, jira_id, team_name, sort_order FROM jira_projects ORDER BY sort_order ASC, id ASC`;
    ...
    if (req.method === 'POST') {
      ...
      try {
        let rows;
        if (id) {
          rows = await sql`UPDATE jira_projects SET jira_id=${jira_id}, team_name=${team_name}, sort_order=${sort_order ?? 0} WHERE id=${id} RETURNING id`;
        } else {
          rows = await sql`INSERT INTO jira_projects (jira_id, team_name, sort_order) VALUES (${jira_id}, ${team_name}, ${sort_order ?? 0}) RETURNING id`;
        }
